...
const getSourceParts = (source: string) => {
    // remove digits, e.g. 'book.authors.2.categoriesx.3.identifier.name' => 'book.authors.categoriesx.identifier.name'
    const sourceWithoutDigits = source.replace(/\.\d+\./g, '.');
    // get final part, e.g. 'book.authors.2.categoriesx.3.identifier.name' => 'identifier.name'
